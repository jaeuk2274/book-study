...
- 다형성을 활용해 계산 코드 재구성하기
  - 공연료 계산기 / interface Calculater -> 상속받아 각 종류별 계산기 Acalculater / Bcalculater / 같은 함수로 타입별 다른 계산 로직 구현
...
- 자주 테스트하라. 작성 중인 코드는 몇 분 간격으로 테스트하고, 적어도 하루에 한 번은 전체 테스트를 돌려보자.

- 테스트는 위험요인을 중심으로 작성해야 한다!
  - 단순히 읽고 쓰는 접근자 등은 굳이 테스트할 필요가 없다.

- 완벽하게 만드느라 테스트를 수행하지 못하느니, 불완전한 테스트라도 작성해 실행하는게 낫다.

- 문제가 생길 가능성이 있는 경계 조건을 생각해보고 집중적으로 테스트
...
- 함수 추출하기
  - 목적과 구현을 분리
    - 코드를 보고 무슨 일을 하는지 파악하는데 한참이 걸린다면, 함수로 추출한 뒤 '무슨 일'에 걸맞는 이름을 짓는다.
    - 목적이 드러나는 이름. '어떻게' 가 아닌 '무엇을' 하는지가 드러나야 한다  

- 함수 선언 바꾸기
  - 주석을 이용해 함수의 목적을 설명해보는 방법 -> 주석이 멋진 이름으로 되돌아올 때가 있다.
  - 마이그레이션 방법
...
- 기본형을 객체로 바꾸기
  ...
  - 코드베이스에 미치는 효과는 놀라울 만큼 크다
  
- 임시 변수를 질의 함수로 바꾸기
  -getPrice / getDiscountFactor
...
- 위임 숨기기
  ...
  - => aPertson.manager; / get manger() {return this.department.manager;}
  
- 중개자 제거하기 (<-> 위임 숨기기)
  - 클라이언트가 위임 객체의 또 다른 기능을 사용하고 싶을 때마다 위임 메서드를 추가해야 하는데
  - 이렇게 기능을 추가하다 보면 단순 전달만 하는 위임 메서드들이 점점 성가셔진다.
  - 단순 중개자 역할로 전락할 때
  - 위임 메서드가 많아지면 사람 클래스의 상당 부분이 그저 위임하는 데만 쓰일 것. 
  - 그럴 때는 중개자를 제거하는 편이 낫다. / 정답은 없다.
  
- 알고리즘 교체하기
  - 더 간명한 방법을 찾아냈을 때
  - 때로는 알고리즘 전체를 걷어내고, 훨씬 간결한 알고리즘
...
- 반복문 쪼개기
  - 종종 반복문 하나에서 두 가지 일을 수행한다. 두 가지 일을 한꺼번에 처리할 수 있다는 이유
  - 하지만 반복문을 수정해야 할 때마다 두 가지 일 모두를 잘 이해하고 진행해야 한다.
  - 반대로 각각의 반복문으로 분리해두면 수정할 동작 하나만 이해하면 된다.
  - 반복문을 두 번 실행해야 하는데?
  - -> <b>리팩터링과 최적화를 구분하자. </b>
  - 최적화는 코드를 깔끔히 정리한 이후 수행. 두 번 실행하는게 병목이라 함은 그때 다시 하나로 합치기는 식은 죽 먹기.
  - 오히려 다른 더 강력한 최적화를 적용할 길을 열어주기도 한다!
  - 더 나아가 쪼갠 반복문을 각각의 함수로 추출
...
### 09.데이터 조직화  
- 변수 쪼개기
  - 역할 하나당 변수 하나 / 여러 용도로 쓰인 변수는 읽는 이에게 혼란을 준다
  - 대입이 두 번 이상 이뤄진다면 여러 가지 역할을 수행한다는 신호다.
  - 역할이 둘 이상인 변수라면 쪼개야 한다.

- 필드 이름 바꾸기
  - 필드 이름들 / 데이터 구조는 프로그램을 이해하는데 정말 큰 역할
  
- 파생 변수를 질의 함수로 바꾸기

- 참조를 값으로 바꾸기 (<-> 값을 참조로 바꾸기)
  - 참조냐 값이냐의 차이는 내부 객체의 속성을 갱신하는 방식에서 가장 극명하게 드러난다.
  - 불변은 외부로 건내줘도 값이 바뀌어 영향을 줄까 걱정하지 않아도 된다.
  - 공유 객체라면(공유 객체의 값이 변경했을때 관련된 객체 모두가 알아야 한다면) 참조로 다뤄야 한다.
...
- 어서션 추가하기
  - 어서션은 프로그램이 어떤 상태임을 가정한 채 실행되는지를 알려주는 훌륭한 소통 도구
...
- 질의 함수와 변경 함수 분리하기
  - 값을 반환하면서, 부수효과도 있는 함수를 발견하면
  - 상태를 변경하는 부분과 질의하는 부분을 분리하려 시도하라!
  
- 플래그 인수 제거하기
  - 호출할 수 있는 함수들이 무엇이고 어떻게 호출해야 하는지 이해하기 어려워진다.
  
- 객체 통째로 넘기기
  - 변화에 대응하기 쉽다
  - 하지만 함수가 레코드 자체에 의존하기를 윈치 않을 때는 수행하지 않는다.
  - 어떤 객체들로부터 값 몇 개를 얻은 후 그 값들만으로 하는 로직이 있다면, 그 로직은 객체 안으로 집어넣자
...
- 메서드 올리기 
  - 상위 : 사원 / 하위 : 세일즈, 엔지니어 
  - 개별 클래스에 get name() -> 상위 사원 클래스에서 단일로 처리
...
- 메서드/필드 내리기
  - 특정 서브클래스 소수와 관련된 메서드/필드
  
- 타입 코드를 서브클래스로 바꾸기
  - 직원, 직원 타입별 다른 로직
  
- 서브클래스 제거하기
  - 서브클래싱은 다름을 프로그래밍하는 멋진 수단이나, 서브클래스로 만든 변종이 다른 모듈로 이동하거나, 사라지면서 가치가 바래지기도 한다.
  
- 슈퍼클래스 추출하기
  - 비슷한 일을 수행하는 두 클래스 -> 상속 메커니즘 활용, 공통의 슈퍼클래스
